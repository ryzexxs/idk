...
const commands = [
  ...
  {
    name: 'lookup',
    description: 'Look up information using VeritasOSINT (Owner only)',
    options: [
      {
        name: 'query',
        type: 3,
        description: 'Email, phone, IP, username, domain, or Discord ID',
        required: true
      }
    ]
  },
  {
    name: 'lookup-email',
    description: 'Look up an email address using VeritasOSINT (Owner only)',
    options: [
      {
        name: 'email',
        type: 3,
  ...
  {
    name: 'lookup-phone',
    description: 'Look up a phone number using VeritasOSINT (Owner only)',
    options: [
      {
        name: 'phone',
        type: 3,
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    try {
      ...
      // VeritasOSINT Lookup Commands (Owner only)
      if (commandName === 'lookup' || commandName === 'lookup-email' || commandName === 'lookup-phone') {
        if (!isOwner(interaction.user.id)) {
          return interaction.reply({
            content: '❌ Only bot owners can use this command.',
            ephemeral: true
        ...
        if (commandName === 'lookup') {
          ...
          lookupType = 'email';
        } else if (commandName === 'lookup-phone') {
          query = interaction.options.getString('phone');
          lookupType = 'phone';
        }

        if (!query) {
          return interaction.editReply({ content: '❌ Please provide a valid query.' });
        }
        ...
        try {
          ...
          const mainEmbed = new EmbedBuilder()
            ...
            .setFooter({ text: 'Powered by VeritasOSINT' })
            .setTimestamp();

          // Add provider results (up to 10 due to Discord limits)
          ...
          for (let i = 0; i < Math.min(results.length, 10); i++) {
            ...
            if (lookupType === 'email' || lookupType === 'phone') {
              ...
              if (providerData.name) {
                value += `**Name:** ${providerData.name}\n`;
              }
              if (providerData.location) {
                value += `**Location:** ${providerData.location}\n`;
              }
              if (providerData.carrier) {
                value += `**Carrier:** ${providerData.carrier}\n`;
              }
              if (providerData.line_type) {
              ...
              if (providerData.city) {
                value += `**City:** ${providerData.city}\n`;
              }
              if (providerData.isp) {
                value += `**ISP:** ${providerData.isp}\n`;
              }
              if (providerData.org) {
              ...
              if (providerData.created_date) {
                value += `**Created:** ${providerData.created_date}\n`;
              }
              ...
              if (providerData.discriminator) {
                value += `**Discriminator:** ${providerData.discriminator}\n`;
              }
              if (providerData.badges) {
                value += `**Badges:** ${Array.isArray(providerData.badges) ? providerData.badges.join(', ') : providerData.badges}\n`;
              }
            }

            if (!value) {
              value = 'Data available (see attachment or full report)';
            }
          ...
          // If there's more data, create additional embeds
          if (results.length > 10) {
            const moreEmbed = new EmbedBuilder()
              .setColor(0x5865F2)
              .setTitle('📊 Additional Results')
              .setDescription(`*${results.length - 10} more providers found. Check the full data above.*`)
              .setFooter({ text: `Total: ${results.length} providers` });
            embeds.push(moreEmbed);
          }

          return interaction.editReply({
            content: `✅ **Lookup Complete**\n\nQuery: \`${data.query}\` | Type: \`${data.type}\` | Results: \`${data.totalResults}\``,
            embeds
          });
          ...
          return interaction.editReply({
            content: `❌ **Lookup Error**\n\n${error.message || 'Failed to query VeritasOSINT'}`
          });
        }
      }
